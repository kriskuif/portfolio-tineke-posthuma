(() => {
  ...
  style.textContent = `
    ...
    .owner-user-list{display:grid;gap:10px;margin-top:12px}
    ...
    .owner-user-row.is-disabled{background:#f4f6f4;border-color:#dde1de}
    ...
    .owner-user-row.is-disabled .owner-user-name{color:#69736e}
    ...
    .owner-user-controls{display:grid;gap:8px;min-width:205px}
    .owner-switch-row{display:flex;align-items:center;justify-content:space-between;gap:12px;font-size:.71rem;font-weight:800;color:#52655b}
    .owner-switch-row.is-disabled{opacity:.5}
    .owner-switch{position:relative;display:inline-flex;align-items:center;flex:0 0 auto;width:42px;height:24px;cursor:pointer}
    .owner-switch input{position:absolute;opacity:0;width:1px;height:1px;pointer-events:none}
    ...
    .owner-switch input:checked~.owner-switch-track{background:#2e7258;box-shadow:inset 0 0 0 1px rgba(24,81,59,.22)}
    .owner-switch input:checked~.owner-switch-thumb{transform:translateX(18px)}
    .owner-switch input:focus-visible~.owner-switch-track{outline:3px solid rgba(46,114,88,.2);outline-offset:2px}
    .owner-switch.is-disabled{cursor:not-allowed;opacity:.45}
    .owner-user-status{font-size:.65rem;color:#78857e;font-weight:750;text-align:right}
    .owner-user-status.on{color:#2f6e56}
    .owner-user-status.off{color:#8a6762}
    .owner-user-actions{display:flex;justify-content:flex-end;margin-top:2px}
    ...
    .owner-user-protected{font-size:.66rem;color:#75827b;font-weight:800;text-align:right}
    ...
    @media(max-width:720px){
      ...
      .owner-user-controls{min-width:0}
      .owner-user-status,.owner-user-protected{text-align:left}
      .owner-user-actions{justify-content:flex-start}
    }
  `;
  ...
  function makeSwitch({checked=false,disabled=false,label,onChange}){
    ...
    wrapper.className = `owner-switch${disabled ? ' is-disabled' : ''}`;
    wrapper.title = label;
    ...
    track.className = 'owner-switch-track';
    const thumb = document.createElement('span');
    thumb.className = 'owner-switch-thumb';
  ...
  function renderUsers(section, users){
    ...
    (users || []).forEach(user=>{
      ...
      const readReceipts = user.read_receipts_enabled !== false;
      const row = document.createElement('div');
      row.className = `owner-user-row${active ? '' : ' is-disabled'}`;
      ...
      controls.className = 'owner-user-controls';

      const activeRow = document.createElement('div');
      activeRow.className = 'owner-switch-row';
      const activeLabel = document.createElement('span');
      activeLabel.textContent = 'Account actief';
      const activeSwitch = makeSwitch({
        checked: active,
        disabled: user.role === 'owner',
        label: user.role === 'owner' ? 'Eigenaar-account is altijd actief' : `Account ${active ? 'uitschakelen' : 'inschakelen'}`,
        onChange: async value=>{
          try{
            await invokeManager({action:'set_active',email:user.email,value});
            if(message) message.textContent = value
              ? `${user.display_name || user.email} is ingeschakeld.`
              : `${user.display_name || user.email} is uitgeschakeld. Berichten en leesbevestigingen zijn tijdelijk verborgen.`;
            await loadUsers(section);
          }catch(err){
            console.error('Accountstatus aanpassen mislukt:',err);
            if(message) message.textContent='Accountstatus aanpassen mislukt: '+(err?.message || 'Onbekende fout');
            throw err;
          }
        }
      });
      activeRow.append(activeLabel,activeSwitch.wrapper);
      controls.appendChild(activeRow);

      const receiptRow = document.createElement('div');
      receiptRow.className = `owner-switch-row${active ? '' : ' is-disabled'}`;
      const receiptLabel = document.createElement('span');
      receiptLabel.textContent = 'Leesbevestigingen';
      const receiptSwitch = makeSwitch({
        checked: readReceipts,
        disabled: !active,
        label: active ? `Leesbevestigingen ${readReceipts ? 'uitschakelen' : 'inschakelen'}` : 'Schakel het account eerst in',
        onChange: async value=>{
          try{
            await invokeManager({action:'set_read_receipts',email:user.email,value});
            if(message) message.textContent = value
              ? `Leesbevestigingen voor ${user.display_name || user.email} staan aan.`
              : `Leesbevestigingen voor ${user.display_name || user.email} staan uit.`;
            await loadUsers(section);
          }catch(err){
            console.error('Leesbevestigingen aanpassen mislukt:',err);
            if(message) message.textContent='Leesbevestigingen aanpassen mislukt: '+(err?.message || 'Onbekende fout');
            throw err;
          }
        }
      });
      receiptRow.append(receiptLabel,receiptSwitch.wrapper);
      controls.appendChild(receiptRow);

      const status = document.createElement('div');
      status.className = `owner-user-status ${active ? 'on' : 'off'}`;
      status.textContent = active
        ? (readReceipts ? 'Actief · leesbevestigingen aan' : 'Actief · leesbevestigingen uit')
        : 'Uitgeschakeld';
      ...
      if(user.role === 'owner'){
        ...
        protectedText.textContent = 'Eigenaar-account beschermd';
        controls.appendChild(protectedText);
      }else{
    ...
    if(message && !message.textContent) message.textContent = users?.length ? `${users.length} beheeraccounts` : 'Geen beheeraccounts gevonden.';
  }
  ...
  function showDeleteBox(row, user, section){
    ...
    text.textContent = `${user.display_name || user.email} definitief verwijderen? Het account en de beheerrechten worden verwijderd. Leesbevestigingen van deze gebruiker worden altijd opgeruimd.`;
  ...
  async function injectOwnerSection(overlay){
    ...
    section.innerHTML = `
      <div class="owner-management-head">
        <div><h4>Eigenaar & gebruikersbeheer</h4><p>Alleen zichtbaar voor het eigenaar-account. Hier kun je beheeraccounts bekijken, tijdelijk uitschakelen, leesbevestigingen beheren, aanmaken en verwijderen.</p></div>
        <span class="owner-badge">Alleen eigenaar</span>
      </div>
      <article class="settings-card" data-editor-list-card>
        <h4>Beheeraccounts</h4>
        <p>Een uitgeschakelde beheerder blijft bewaard, maar heeft tijdelijk geen beheerrechten. Berichten en leesbevestigingen van die gebruiker worden verborgen tot het account weer wordt ingeschakeld.</p>
    ...
    section.querySelector('[data-create-editor]')?.addEventListener('click', async()=>{
      ...
      if(!email){ createMessage.textContent='Vul een e-mailadres in.'; return; }
      if(password.length < 8){ createMessage.textContent='Het wachtwoord moet minimaal 8 tekens bevatten.'; return; }
      if(password !== confirm){ createMessage.textContent='De twee wachtwoorden komen niet overeen.'; return; }
      ...
      if(error){
        ...
        createMessage.textContent='Account aanmaken mislukt: ' + detail;
        return;
      }
      ...
      createMessage.textContent='Account is aangemaakt en direct bevestigd. Er kan nu met dit e-mailadres en wachtwoord worden ingelogd.';
      await loadUsers(section);
    });
